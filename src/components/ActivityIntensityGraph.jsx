    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
        <SectionHeader
          ...
          subtitle="Zone-based movement tracking"
          color="#fbbf24"
          actions={
            <div className="flex items-center gap-2">
              <ZoneBadge zone={activityZone} />
              <LiveBadge color="#fbbf24" />
            </div>
          }
        />
      </div>

      {/* Current intensity bar */}
      <div className="mb-5">
        <div className="flex justify-between items-center mb-1.5">
          <span className="text-[10px] font-mono text-gray-500 uppercase">Load</span>
          <motion.span key={currentIntensity}
            initial={{ scale: 1.1 }} animate={{ scale: 1 }}
            className="text-2xl font-bold font-mono" style={{ color: activityZone.color }}>
        ...
        <div className="h-2 bg-white/5 rounded-full overflow-hidden">
          <motion.div
            ...
            style={{
              background: `linear-gradient(90deg, #60a5fa, #34d399, #fbbf24, #f87171, #e879f9)`,
              backgroundSize: '200% 100%',
              backgroundPosition: `${100 - currentIntensity}% 0`,
            }}
          />
        </div>
      </div>
      ...
      <div className="h-36 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            {zoneBreaks.map(y => (
              <ReferenceLine key={y} y={y} stroke="rgba(255,255,255,0.05)" strokeDasharray="3 3" />
            ))}
            <Area
              type="monotone"
              dataKey="value"
      ...
      <div className="mt-3 pt-3 border-t border-white/5">
          ...
          {timeInZones.map(z => (
            <div key={z.name} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full rounded-sm transition-all duration-500"
                style={{ height: `${Math.max(2, z.pct * 0.3)}px`, backgroundColor: z.color, opacity: z.pct > 0 ? 1 : 0.15 }} />
        ...
        <div className="flex gap-2 mt-1">
          {timeInZones.map(z => (
            <span key={z.name} className="flex-1 text-[8px] font-mono text-gray-600 text-center truncate">{z.name}</span>
          ))}
        </div>
      </div>
    </motion.div>
