...
export default function HRVTrendGraph({ hrvHistory, currentHrv }) {
  ...
  return (
    <div className="glass-card rounded-3xl p-5 relative overflow-hidden vitals-slide-up" style={{ animationDelay: '0.1s' }}>
      ...
      <div className="flex justify-between items-start mb-4">
        <SectionHeader icon={Activity} title="HRV Trend" subtitle="RMSSD-derived variability" color="var(--color-pulse-green)" actions={<LiveBadge color="var(--color-pulse-green)" />} />
      </div>
      <div className="flex items-center gap-4 mb-4">
        <div>
          <span key={currentHrv} className="text-4xl font-bold font-mono text-white vitals-pop-in" style={{ textShadow: '0 0 16px rgba(57,255,106,0.5)' }}>{currentHrv}</span>
          <span className="text-xs text-gray-500 font-bold uppercase ml-1.5">ms</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full border" style={{ color: status.color, backgroundColor: `${status.color}15`, borderColor: `${status.color}30` }}>{status.label}</span>
          <span className="flex items-center gap-1 text-[10px] font-medium" style={{ color: trendColor }}><TrendIcon size={10} /> {trend.charAt(0).toUpperCase() + trend.slice(1)}</span>
        </div>
      </div>
      <div className="h-36 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            <defs><filter id="hrv-glow"><feGaussianBlur stdDeviation="2" result="blur" /><feComposite in="SourceGraphic" in2="blur" /></filter></defs>
            ...
            <Line type="monotone" dataKey="value" stroke="var(--color-pulse-green)" strokeWidth={2.5} dot={false} isAnimationActive={false} filter="url(#hrv-glow)" />
          </LineChart>
        </ResponsiveContainer>
      </div>
      <div className="flex gap-3 mt-3 pt-3 border-t border-white/5">
        {[{ label: 'Low <30ms', color: '#FF4560' }, { label: 'Mod 30–60ms', color: '#fbbf24' }, { label: 'High >60ms', color: '#39FF6A' }].map(z => (
          <span key={z.label} className="flex items-center gap-1 text-[9px] font-mono text-gray-500">
            <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: z.color }} />{z.label}
          </span>
        ))}
      </div>
    </div>
