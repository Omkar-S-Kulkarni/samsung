 ...
 * Supports toggle-based comparison mode.
 */
...
import { SectionHeader, ChartToggle } from './VitalsComponents';
...
export default function MultiSignalChart({ hrHistory, hrvHistory, intensityHistory, timeWindow }) {
  ...
  const data = useMemo(() => {
    ...
    return Array.from({ length: len }, (_, i) => {
      ...
      return {
        label: i === len - 1 ? 'Now' : `-${len - 1 - i}s`,
        hr:        hrHistory[idx],
        hrv:       hrvHistory[idx],
        intensity: intensityHistory[idx],
      };
    });
  ...
  return (
    ...
    >
      <div className="absolute top-0 left-0 w-full h-0.5"
        style={{ background: 'linear-gradient(90deg, var(--color-pulse-cyan), var(--color-pulse-green), #fbbf24, transparent)' }} />

      <div className="flex justify-between items-start mb-4">
        <SectionHeader
          icon={BarChart2}
          title="Multi-Signal View"
          subtitle="Overlay all biometric channels"
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {SIGNALS.map(s => (
          <button key={s.id}
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-full border text-[10px] font-bold font-mono uppercase transition-all"
            style={{
              color:           activeSignals.includes(s.id) ? s.color : '#4b5563',
              borderColor:     activeSignals.includes(s.id) ? `${s.color}40` : 'rgba(255,255,255,0.08)',
              backgroundColor: activeSignals.includes(s.id) ? `${s.color}12` : 'transparent',
            }}>
            <span className="w-2 h-2 rounded-full" style={{ backgroundColor: s.color, opacity: activeSignals.includes(s.id) ? 1 : 0.3 }} />
            {s.label}
          </button>
        ))}
      </div>
      ...
      <div className="h-56 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            {activeSignals.includes('intensity') && (
              <Line
                yAxisId="hrv"
                type="monotone"
                dataKey="intensity"
      ...
      <div className="flex justify-between items-center mt-3 pt-3 border-t border-white/5">
        <div className="flex gap-4">
          {SIGNALS.map(s => (
            <span key={s.id}
              className="flex items-center gap-1.5 text-[9px] font-mono uppercase transition-all"
              style={{ color: activeSignals.includes(s.id) ? s.color : '#4b5563' }}>
              <span className="w-2 h-px inline-block" style={{ backgroundColor: s.color, opacity: activeSignals.includes(s.id) ? 1 : 0.3 }} />
              {s.label} ({s.unit})
            </span>
          ))}
        </div>
