...
export default function StepCountTracker({ steps }) {
  ...
  return (
    <div className="glass-card rounded-3xl p-5 relative overflow-hidden vitals-slide-up" style={{ animationDelay: '0.2s' }}>
      <div className="absolute top-0 left-0 w-0.5 h-full bg-gradient-to-b from-[#c084fc] via-[#c084fc]/30 to-transparent" />
      <div className="flex justify-between items-start mb-5">
        <SectionHeader icon={Footprints} title="Step Tracker" subtitle={`Daily goal: ${GOAL.toLocaleString()} steps`} color="#c084fc" />
        <span className="text-[10px] font-mono px-2 py-1 rounded-full border font-bold" style={{ color: status.color, borderColor: `${status.color}30`, backgroundColor: `${status.color}10` }}>{status.label}</span>
      </div>
      <div className="flex items-center gap-6">
        ...
        <div className="flex-1 flex flex-col gap-3">
          <div>
            ...
            <div className="text-3xl font-bold font-mono text-white"><AnimatedValue value={steps} /></div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-white/5 rounded-xl p-2 border border-white/5">
              ...
              <div className="text-sm font-bold font-mono mt-0.5" style={{ color: '#fbbf24' }}>~{getCalorieEstimate(steps)} kcal</div>
            </div>
            <div className="bg-white/5 rounded-xl p-2 border border-white/5 col-span-2">
              ...
              <div className="text-sm font-bold font-mono mt-0.5" style={{ color: '#c084fc' }}>~{getDistanceEstimate(steps)} km</div>
            </div>
          </div>
        </div>
      </div>
      <div className="mt-5">
        ...
        <div className="h-2 bg-white/5 rounded-full overflow-hidden">
          <div className="h-full rounded-full transition-all duration-800 ease-out" style={{ width: `${pct}%`, background: 'linear-gradient(90deg, #7c3aed, #c084fc, #e879f9)' }} />
        </div>
        <div className="flex justify-between mt-1 px-0.5">
          {[25, 50, 75, 100].map(m => (
            <div key={m} className="flex flex-col items-center gap-0.5">
              <div className="w-px h-1.5" style={{ backgroundColor: steps / GOAL * 100 >= m ? '#c084fc' : 'rgba(255,255,255,0.1)' }} />
              <span className="text-[8px] font-mono" style={{ color: steps / GOAL * 100 >= m ? '#c084fc' : '#4b5563' }}>{(GOAL * m / 100 / 1000).toFixed(1)}k</span>
            </div>
          ))}
        </div>
      </div>
